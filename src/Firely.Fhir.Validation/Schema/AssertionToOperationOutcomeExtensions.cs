...
public static class AssertionToOperationOutcomeExtensions
{
    ...
    {
        builder ??= (_, code, type, severity) => Issue.Create(code, severity, type);
        var outcome = new OperationOutcome();

        foreach (var item in result.Evidence.OfType<IssueAssertion>())
        {
            var issue = builder(item.Location, item.IssueNumber, item.Type ?? IssueType.Unknown, item.Severity);
            ...
            annotateIssueExtensions(item, newIssueComponent);
        }

        return outcome;
    }

    private static void annotateIssueExtensions(IssueAssertion item, IssueComponent newIssueComponent)
    {
        var q = item.DefinitionPath?.Current;
        while (q is not null)
        {
            // annotate only if we're on a type with a non-default profile
