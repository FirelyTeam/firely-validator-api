...
namespace Firely.Fhir.Validation
{
    public static class ValidationExtensions
    {
        ...
        public static async Task<Assertions> Validate(IElementSchemaResolver resolver, Uri uri, IEnumerable<ITypedElement> input, ValidationContext vc)
        {
            var schema = await resolver.GetSchema(uri).ConfigureAwait(false);
            return await schema.Validate(input, vc).ConfigureAwait(false);
        }

        public static async Task<Assertions> Validate(IElementSchemaResolver resolver, Uri uri, ITypedElement input, ValidationContext vc)
        {
            var schema = await resolver.GetSchema(uri).ConfigureAwait(false);
            return await schema.Validate(input, vc).ConfigureAwait(false);
        }
