...
{
    public class ScopedNode2 : ITypedElement, IAnnotated, IExceptionSource
    {
        private class Cache
        {
        ...
        public ScopedNode2(ITypedElement wrapped)
        {
            Current = wrapped;
        ...
        private ScopedNode2(ScopedNode2 parent, ScopedNode2? parentResource, ITypedElement wrapped, string? fullUrl)
        {
            Current = wrapped;
        ...
        public bool AtResource => Current is IResourceTypeSupplier rt && rt.ResourceType is not null;
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<ScopedNode2> ParentResources()
        {
        ...
        public IEnumerable<ScopedNode2> ContainedResources()
        {
            if (_cache.ContainedResources == null)
            {
                _cache.ContainedResources = AtResource ?
                    this.Children("contained").Cast<ScopedNode2>() :
                    Enumerable.Empty<ScopedNode2>();
            }
            return _cache.ContainedResources;
        }

        public record BundledResource(string FullUrl, ScopedNode2 Resource);
            ...
            if (_cache.BundledResources == null)
            {
                if (InstanceType == "Bundle")
                    _cache.BundledResources = from e in this.Children("entry")
                                              ...
                                              let resource = e.Children("resource").FirstOrDefault() as ScopedNode2
                                              select new BundledResource(fullUrl, resource);
                else
