...
public class BindingValidatorTests
{
    ...
    [TestMethod]
    public void ValidateConceptWithExtensibleBinding()
    {
        var cc = new CodeableConcept().ToPocoNode(); // without codes nor text.

        // Required binding should result in an error
        var result = _bindingAssertion.Validate(cc, _validationSettingsM);
        Assert.IsFalse(result.IsSuccessful);
        result.Errors.Count.Should().Be(1);
        result.Warnings.Count.Should().Be(0);

        // Extensible binding should result in a warning
        var bindingWithExtBinding =
            new BindingValidator(_bindingAssertion.ValueSetUri, BindingValidator.BindingStrength.Extensible);
        result = bindingWithExtBinding.Validate(cc, _validationSettingsM);
        Assert.IsTrue(result.IsSuccessful);
