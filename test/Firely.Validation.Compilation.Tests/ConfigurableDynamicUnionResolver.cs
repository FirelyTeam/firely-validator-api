...
{
    internal class MessagePackSerializerOptionsInternal : MessagePackSerializerOptions
    {
        protected internal MessagePackSerializerOptionsInternal(IFormatterResolver resolver) : base(resolver)
        {
        }
    }
    ...
    /// </summary>
    public sealed class ConfigurableDynamicUnionResolver : IFormatterResolver
    {
        private static Dictionary<Type, List<Type>> AttributesByType { get; } = new Dictionary<Type, List<Type>>();

        public static void DeclareUnion(Type baseType, params Type[] derivedTypes) =>
            DeclareUnion(baseType, (IEnumerable<Type>)derivedTypes);

        public static void DeclareUnion(Type baseType, IEnumerable<Type> derivedTypes)
        {
        ...
        /// </summary>
        public static readonly ConfigurableDynamicUnionResolver Instance;
        ...
        static ConfigurableDynamicUnionResolver()
        {
            Instance = new ConfigurableDynamicUnionResolver();
            Options = new MessagePackSerializerOptionsInternal(Instance);
            DynamicAssembly = new Lazy<DynamicAssembly>(() => new DynamicAssembly(ModuleName));
        }

        private ConfigurableDynamicUnionResolver()
        {
        }
        ...
        private static class FormatterCache<T>
        {
            ...
            static FormatterCache()
            {
                ...
                if (ti.IsNullable())
                {
                    ...
                    var innerFormatter = ConfigurableDynamicUnionResolver.Instance.GetFormatterDynamic(ti.AsType());
        ...
        private static TypeInfo BuildType(Type type)
        {
            ...
            var typeAttributes = ti.GetCustomAttributes<UnionAttribute>();
            var additionalAttributes = AttributesByType.TryGetValue(type, out var x) ? x : new List<Type>();
            var indexedAdditionalAttributes = additionalAttributes.Select((att, ix) => new UnionAttribute(ix, att));
            UnionAttribute[] unionAttrs = typeAttributes.Concat(indexedAdditionalAttributes).OrderBy(x => x.Key).ToArray();
